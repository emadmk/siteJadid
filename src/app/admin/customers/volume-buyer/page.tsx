...
import { Users, Search, Mail, Phone, Eye, ArrowLeft, Building2 } from 'lucide-react';
import { db } from '@/lib/db';

async function getVolumeBuyerCustomers(searchParams: { search?: string; status?: string }) {
  const where: any = {
    accountType: { in: ['B2B', 'VOLUME_BUYER'] },
  };

  if (searchParams.status) {
    where.approvalStatus = searchParams.status;
  }
...
async function getVolumeBuyerStats() {
  const [total, approved, pending, rejected] = await Promise.all([
    db.user.count({ where: { accountType: { in: ['B2B', 'VOLUME_BUYER'] } } }),
    db.user.count({
      where: { accountType: { in: ['B2B', 'VOLUME_BUYER'] }, approvalStatus: 'APPROVED' },
    }),
    db.user.count({
      where: { accountType: { in: ['B2B', 'VOLUME_BUYER'] }, approvalStatus: 'PENDING' },
    }),
    db.user.count({
      where: { accountType: { in: ['B2B', 'VOLUME_BUYER'] }, approvalStatus: 'REJECTED' },
    }),
  ]);
...
export default async function VolumeBuyerCustomersPage({
  searchParams,
}: {
  searchParams: { search?: string; status?: string };
}) {
  const [customers, stats] = await Promise.all([
    getVolumeBuyerCustomers(searchParams),
    getVolumeBuyerStats(),
  ]);
    ...
    <div className="p-8">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">Business and bulk order accounts</p>
          </div>
          {stats.pending > 0 && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="text-sm text-gray-600">Total Volume Buyers</div>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          {customers.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <h3 className="text-lg font-medium text-black mb-2">
                No volume buyers found
              </h3>
              <p className="text-gray-600">
                {searchParams.search || searchParams.status
                  ? 'Try adjusting your filters'
                  : 'No volume buyers registered yet'}
              </p>
            </div>
          ) : (
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${
                            approvalStatusColors[customer.approvalStatus || 'PENDING'] || 'bg-gray-100 text-gray-800'
                          }`}
                        >
                          {customer.approvalStatus || 'N/A'}
                        </span>
                      </td>
                      <td className="px-6 py-4">
