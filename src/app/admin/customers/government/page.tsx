...
import { Shield, Search, Mail, Phone, Eye, ArrowLeft, Building2 } from 'lucide-react';
import { db } from '@/lib/db';

async function getGovernmentCustomers(searchParams: { search?: string; status?: string }) {
  const where: any = {
    accountType: { in: ['GSA', 'GOVERNMENT'] },
  };

  if (searchParams.status) {
    where.OR = [
      { approvalStatus: searchParams.status },
      { gsaApprovalStatus: searchParams.status },
    ];
  }
  ...
  if (searchParams.search) {
    ...
    where.AND.push({
      OR: [
        ...
        { governmentDepartment: { contains: searchParams.search, mode: 'insensitive' } },
        { gsaDepartment: { contains: searchParams.search, mode: 'insensitive' } },
      ],
    });
  }
...
async function getGovernmentStats() {
...
export default async function GovernmentCustomersPage({
  searchParams,
}: {
  searchParams: { search?: string; status?: string };
}) {
  const [customers, stats] = await Promise.all([
    getGovernmentCustomers(searchParams),
    getGovernmentStats(),
  ]);
    ...
    <div className="p-8">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">Federal and government agency accounts</p>
          </div>
          {stats.pending > 0 && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="text-3xl font-bold text-safety-green-600 mb-1">{stats.total}</div>
          <div className="text-sm text-gray-600">Total Government</div>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="md:col-span-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search by name, email, agency, or department..."
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-gray-600">
                {searchParams.search || searchParams.status
                  ...
                  : 'No government customers registered yet'}
              </p>
            </div>
          ) : (
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {customers.map((customer) => {
                  ...
                  const department = customer.governmentDepartment || customer.gsaDepartment;
                  const status = customer.approvalStatus || customer.gsaApprovalStatus || 'PENDING';
                        ...
                        <div className="space-y-1">
                          {customer.companyName && (
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm text-gray-700">{customer.companyName}</span>
                            </div>
                          )}
                          {department && (
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${
                            approvalStatusColors[status] || 'bg-gray-100 text-gray-800'
                          }`}
                        >
                          {status}
                        </span>
