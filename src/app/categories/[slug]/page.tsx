...
export default function CategoryPage({ params }: { params: { slug: string } }) {
  ...
  const fetchData = useCallback(async (page: number, reset: boolean = false) => {
    ...
    try {
      ...
      if (reset || page === 1) {
        ...
        // Initialize expanded sections for smart filters
        const expanded: Record<string, boolean> = {};
        Object.keys(result.smartFilters || {}).forEach((key, index) => {
          expanded[key] = index < 3; // First 3 sections expanded by default
        });
  ...
  // Infinite scroll for pages 1-4, then show pagination
  useEffect(() => {
                    ...
                    <div key={filterKey}>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {values.slice(0, 5).map((value) => (
                          <button
                            key={value}
                            onClick={() => toggleSmartFilter(filterKey, value)}
                            className={`px-3 py-1 text-sm rounded-full border transition-colors ${
                              (activeSmartFilters[filterKey] || []).includes(value)
                                ? 'bg-safety-green-600 text-white border-safety-green-600'
                                : 'border-gray-300 text-gray-700 hover:border-safety-green-500'
                            }`}
                          >
                            {value}
                          </button>
