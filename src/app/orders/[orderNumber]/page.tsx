...
export default async function OrderDetailPage({
  ...
  const paymentSucceeded = redirectStatus === 'succeeded';
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8 max-w-5xl">
        ...
        {/* New Order (non-card payment) */}
        {isNewOrder && !redirectStatus && (
          <div className="bg-green-50 border-2 border-green-200 rounded-xl p-6 mb-8">
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 bg-green-600 rounded-full flex items-center justify-center flex-shrink-0">
                <CheckCircle className="w-6 h-6 text-white" />
        ...
        {isNewOrder && paymentSucceeded && order.paymentStatus === 'PENDING' && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6 flex items-center gap-3">
            <Loader2 className="w-5 h-5 text-blue-600 animate-spin" />
            <span className="text-blue-800 text-sm">
              Payment is being confirmed. This page will update automatically.
            </span>
          </div>
        )}
...
export async function generateMetadata({ params }: { params: { orderNumber: string } }) {
  return {
    title: `Order ${params.orderNumber} | ADA Supplies`,
