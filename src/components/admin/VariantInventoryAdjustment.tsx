...
interface VariantInventoryAdjustmentProps {
  variantId: string;
  variantName: string;
  productName: string;
  currentStock: number;
...
export function VariantInventoryAdjustment({
  variantId,
  variantName,
  productName,
  currentStock,
}: VariantInventoryAdjustmentProps) {
  ...
  const handleAdjustment = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/variants/${variantId}/inventory`, {
        method: 'PUT',
        headers: {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-black">Adjust Variant Inventory</h3>
          <button
            onClick={() => {
              setIsOpen(false);
              setError('');
        ...
        <div className="mb-4 p-3 bg-gray-50 rounded-lg">
          ...
          <div className="text-xs text-gray-500 mb-1">Variant: {variantName}</div>
